    ...
    def CreatePromptCoordinates():
        ...
        for j in range(len(data_geojson)):
            for k in  range( j , len(data_geojson)):
                if data_geojson["features"][k]["distance"]>data_geojson["features"][j]["distance"]:
        ...
        position_query = PromptTemplate(template = position_template, input_variables=["location","question"])
